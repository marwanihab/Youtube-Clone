...
const module_session_info_1 = require("./module-session-info");
...
class GraphQLModule {
    ...
    mock(partialCache) {
        if (partialCache.contextBuilder) {
            ...
            partialCache.contextBuilder = async (session, excludeSession) => {
                session = helpers_1.normalizeSession(session);
                const moduleSessionInfo = new module_session_info_1.ModuleSessionInfo(this, session);
        ...
        if (typeof this._cache.schema === 'undefined') {
            ...
            try {
                ...
                if (importsSchemas.length || selfTypeDefs || selfExtraSchemas.length) {
                    this._cache.schema = graphql_toolkit_1.mergeSchemas({
                    ...
                    try {
                        if (!this._cache.schema) {
                            ...
                            try {
                                ...
                                const selfResolverValidationOptions = this.selfResolverValidationOptions;
                                if (importsSchemas.length || selfTypeDefs || selfExtraSchemas.length) {
                                    this._cache.schema = await graphql_toolkit_1.mergeSchemasAsync({
    ...
    get injector() {
        if (typeof this._cache.injector === 'undefined') {
            this.checkConfiguration();
            const injector = (this._cache.injector = new di_1.Injector({
                name: this.name,
                injectorScope: di_1.ProviderScope.Application,
    ...
    get typeDefs() {
        if (typeof this._cache.typeDefs === 'undefined') {
            ...
            if (typeDefsArr.length) {
                this._cache.typeDefs = graphql_toolkit_1.mergeTypeDefs(typeDefsArr, {
                    exclusions: this._exclusionsFromSchema,
            ...
            if (typeof this._cacheAsync.typeDefsAsync) {
                this._cacheAsync.typeDefsAsync = new Promise(async (resolve, reject) => {
                    try {
                        ...
                        if (typeDefs.length) {
                            this._cache.typeDefs = graphql_toolkit_1.mergeTypeDefs(typeDefs.filter(s => s), {
                                exclusions: this._exclusionsFromSchema,
                                useSchemaDefinition: false
                            });
                        }
    ...
    get resolvers() {
        if (typeof this._cache.resolvers === 'undefined') {
            ...
            const composedResolvers = graphql_toolkit_1.composeResolvers(graphql_toolkit_1.mergeResolvers(resolversToBeComposed, {
                exclusions: this._exclusionsFromSchema
    ...
    get schemaDirectives() {
        if (typeof this._cache.schemaDirectives === 'undefined') {
            ...
            for (const directiveName in selfDirectiveResolvers) {
                selfSchemaDirectives[directiveName] = graphql_toolkit_1.getSchemaDirectiveFromDirectiveResolver(selfDirectiveResolvers[directiveName]);
            }
    ...
    get selfDefaultProviderScope() {
        let defaultProviderScope = di_1.ProviderScope.Application;
        ...
        if (typeDefsDefinitions) {
            ...
            if (typeof typeDefsDefinitions === 'string') {
                ...
                if (typeDefsDefinitions.length) {
                    typeDefs = graphql_toolkit_1.mergeTypeDefs(typeDefsDefinitions, {
                        useSchemaDefinition: false
                    });
                }
            }
            else if (typeDefsDefinitions instanceof graphql_1.GraphQLSchema) {
                typeDefs = graphql_1.parse(graphql_toolkit_1.printSchemaWithDirectives(typeDefsDefinitions));
            }
    ...
    get selfResolvers() {
        ...
        `);
            ...
            if (Array.isArray(resolversDefinitions)) {
                resolversDefinitions = graphql_toolkit_1.mergeResolvers(resolversDefinitions);
            }
    ...
    get selfImports() {
        ...
        if (imports.find(module => typeof module === 'undefined')) {
            throw new errors_1.DependencyModuleUndefinedError(this.name);
        }
        return imports;
    }
    get selfProviders() {
    ...
    _isScalarResolver(resolver) {
        return (resolver instanceof graphql_1.GraphQLScalarType ||
    ...
    addSessionInjectorToSelfResolversCompositionContext(resolversComposition) {
        ...
        for (const path in resolversComposition) {
            if (resolversComposition[path] instanceof Function || resolversComposition[path] instanceof Array) {
                const compositionArr = helpers_1.asArray(resolversComposition[path]);
                resolversComposition[path] = visitResolversCompositionElem(compositionArr);
            }
            else {
                for (const subPath in resolversComposition[path]) {
                    const compositionArr = helpers_1.asArray(resolversComposition[path][subPath]);
...
GraphQLModule.defaultCache = new apollo_server_caching_1.InMemoryLRUCache();
